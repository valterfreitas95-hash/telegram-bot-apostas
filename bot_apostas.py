...
def enviar_telegram(msg: str):
    """Envia mensagem usando a API oficial via requests."""
...
def buscar_jogos_modelo_c():
    ...
    print(f"✅ Jogos encontrados: {len(selecionados)}")
    return selecionados
...
def montar_mensagem(lista):
    ...
    if not lista:
        return (
            ...
            f"Data: {hoje}\n\n"
            f"⚠️ Nenhum jogo corresponde aos critérios hoje.\n"
            f"🎯 Critérios:\n"
            f"- Odd da casa ≤ {MAX_ODD}\n"
            f"- Probabilidade ≥ {MIN_PROB*100:.0f}%"
        )

    msg = (
        f"📊 *Apostas promissoras do dia (Modelo C)*\n"
        f"Data: {hoje}\n"
        f"🎯 Critérios: odd ≤ {MAX_ODD} OU prob ≥ {MIN_PROB*100:.0f}%\n\n"
    )
...
@app.route("/")
def index():
    return "BOT DE APOSTAS ONLINE — MODELO C", 200
...
if __name__ == "__main__":
    ...
    print(f"🌐 Servidor Flask na porta {porta}...")
